...
ERROR_STATUS SPI_Init(SPI_Cfg_s* pstr_SPI_Confg)
{
    ...
    if (pstr_SPI_Confg->u8_Prescaler == SPI_PRESCALER_128)
    {
        SPI_STATUS_REG = (SPI_STATUS_REG & STU_PRESCALER_MASK) | ((pstr_SPI_Confg->u8_Prescaler) % 2);
    }
    ...
    SPI_CTRL_REG = SPI_CTRL_REG | SPI_ENB;
...
ERROR_STATUS SPI_GetStatus(uint8 *u8_Data)
{
    ...
    {
        SPI_STATUS_REG = SPI_STATUS_REG | 0x80;
        *u8_Data = BYTE_SENT;
    }
    return E_OK;
}
