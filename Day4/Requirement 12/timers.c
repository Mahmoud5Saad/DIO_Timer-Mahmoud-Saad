...
{
	uint32 tcnt_ticks = F_CPU/(1000 * freq);
	uint32 tcnt_start = 0xff - tcnt_ticks;
	uint32 ocr_ticks = ( (tcnt_ticks * dutyCycle) / 100) - 1;
	uint32 ocr_start = tcnt_start + ocr_ticks;
...
void timer1SwPWM(uint8 dutyCycle,uint8 freq)
{
	...
	uint32 tcnt_ticks = F_CPU/(1000 * freq);
	uint32 tcnt_start = 0xffff - tcnt_ticks;
	uint32 ocr_ticks = ( (tcnt_ticks * dutyCycle) / 100) - 1;
	uint32 ocr_start = tcnt_start + ocr_ticks;
